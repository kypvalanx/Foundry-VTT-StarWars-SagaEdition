    ...
    quench.registerBatch("swse.prerequisites", (context) => {
        ...
        describe("Prerequisite Engine", () => {
            ...
            describe("Attributes and Dark Side Score", () => {
                ...
                it("should allow adding feats that meet an ability score requirement", async () => {
                    await withTestActor(async (actor) => {

                        await actor.setAttributes({dex: 13});
                        await actor.sheet._onDropItem(getMockEvent(), {name: "Dodge", type: "feat"});
                        // actor.sheet._onDropItem(getMockEvent(), {name: "Soldier", type: "class"});
                        // actor.sheet._onDropItem(getMockEvent(), {name: "Soldier", type: "class"});
                        ...
                        assert.equal(actor.feats.length, 1)
                    })
                })
            });
