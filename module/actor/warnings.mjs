...
export function errorsFromActor(actor) {
    ...
    if(levelEffects.length > 0){
        errors.push(`<span>You have level Effects on your actor sheet.  You may notice strange behavior if you do not remove them from your "Modes" tab</span>`)
    }

    const weaponFireEffects = actor.effects.filter(effect => effect.flags.swse.group === "Fire Mode");
    if(weaponFireEffects.length > 0){
        errors.push(`<span>You have Fire Mode Effects on your actor sheet (Single-Shot, Autofire, etc...).  You may notice strange behavior if you do not remove them from your "Modes" tab</span>`)
    }
