...
import {ShieldFields, ShieldFunctions} from "./templates/shields.mjs";
import {TraitsFunctions} from "./templates/traits.mjs";
import {AttributeFields, AttributeFunctions} from "./templates/attributes.mjs";
...
export class CharacterDataModel extends SystemDataModel.mixin(
	...characterFunctionClasses
) {
	static defineSchema() {
		return {
			...HealthFields.common,
			...ShieldFields.common,
			...AbilityFields.common,
			...AttributeFields.common,
			...
			equippedIds: new fields.ArrayField(new fields.StringField()),
	...
	prepareDerivedData() {
		...
		//Attributes derivation must be done first.
		this._prepareAttributesDerivedData();
		...
		//Traits
		this._prepareTraitsDerivedData();
	...
	#_validateLevelUpOptions() {
		...
		let characterLevel = system.attributes.level.value;
		...
		let classLevel = system.attributes.level.value;
		system.availableItems["General Feats"] =
			1 +
			Math.floor(classLevel / 3) +
			(system.availableItems["General Feats"]
				? system.availableItems["General Feats"]
				: 0);
